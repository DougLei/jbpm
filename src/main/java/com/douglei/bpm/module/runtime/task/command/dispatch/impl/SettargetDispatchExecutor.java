...
public class SettargetDispatchExecutor extends DispatchExecutor {
	...
	public void execute() throws TaskNotExistsException, TaskDispatchException {
		...
		if(activateLastAssigneeList) {
			List<Object[]> list = SessionContext.getSqlSession().limitQuery_(1, 1, "select taskinst_id from bpm_hi_task where procinst_id=? and key_=? order by end_time desc", Arrays.asList(handleParameter.getProcessInstanceId(), targetTask));
			if(list.size() == 1)
